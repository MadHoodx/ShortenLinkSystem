...
export default {
  ...
  const regPasswordConfirm = ref('');
    ...
    async function onLogin() {
      ...
      try {
        const res = await axios.post(`${props.apiBase}/api/auth/login`, { email: email.value, password: password.value }, { withCredentials: true });
    ...
    async function onRegister() {
      ...
      try {
        const res = await axios.post(`${props.apiBase}/api/auth/register`, { email: regEmail.value, password: regPassword.value }, { withCredentials: true });
        if (res.status === 201) {
